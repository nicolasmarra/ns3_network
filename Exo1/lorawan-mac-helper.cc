...
void LorawanMacHelper::Set(std::string name, const AttributeValue &v) {
    m_mac.Set(name, v);
}

void LorawanMacHelper::SetDeviceType(enum DeviceType dt) {
...
void LorawanMacHelper::SetAddressGenerator(
...
void LorawanMacHelper::SetRegion(enum LorawanMacHelper::Regions region) {
    m_region = region;
}

Ptr<LorawanMac> LorawanMacHelper::Create(Ptr<Node> node,
    ...
    if (m_deviceType == ED_A && m_addrGen) {
        mac->GetObject<ClassAEndDeviceLorawanMac>()->SetDeviceAddress(
    ...
    if (m_deviceType == ED_A) {
        Ptr<ClassAEndDeviceLorawanMac> edMac =
            mac->GetObject<ClassAEndDeviceLorawanMac>();
        switch (m_region) {
...
void LorawanMacHelper::ConfigureForAlohaRegion(
...
void LorawanMacHelper::ConfigureForAlohaRegion(
    Ptr<GatewayLorawanMac> gwMac) const {
    NS_LOG_FUNCTION_NOARGS();
...
void LorawanMacHelper::ApplyCommonAlohaConfigurations(
...
void LorawanMacHelper::ConfigureForEuRegion(
    Ptr<ClassAEndDeviceLorawanMac> edMac) const {
    NS_LOG_FUNCTION_NOARGS();
...
void LorawanMacHelper::ApplyCommonEuConfigurations(
    Ptr<LorawanMac> lorawanMac) const {
    NS_LOG_FUNCTION_NOARGS();
...
void LorawanMacHelper::ConfigureForSingleChannelRegion(
    Ptr<ClassAEndDeviceLorawanMac> edMac) const {
    NS_LOG_FUNCTION_NOARGS();
...
void LorawanMacHelper::ApplyCommonSingleChannelConfigurations(
    Ptr<LorawanMac> lorawanMac) const {
    NS_LOG_FUNCTION_NOARGS();
...
std::vector<int> LorawanMacHelper::SetSpreadingFactorsUp(
    NodeContainer endDevices, NodeContainer gateways, Ptr<LoraChannel> channel,
    uint8_t codingRate, bool crc, uint8_t mtype) {
    NS_LOG_FUNCTION_NOARGS();
    ...
    for (auto j = endDevices.Begin(); j != endDevices.End(); ++j) {
        ...
        Ptr<LoraNetDevice> loraNetDevice =
            netDevice->GetObject<LoraNetDevice>();
        ...
        // Try computing the distance from each gateway and find the best
        // one
        Ptr<Node> bestGateway = gateways.Get(0);
        Ptr<MobilityModel> bestGatewayPosition =
            bestGateway->GetObject<MobilityModel>();

        // Assume devices transmit at 14 dBm
        double highestRxPower =
            channel->GetRxPower(14, position, bestGatewayPosition);

        for (auto currentGw = gateways.Begin() + 1; currentGw != gateways.End();
             ++currentGw) {
            ...
            double currentRxPower =
                channel->GetRxPower(14, position, currPosition); // dBm
        ...
        Ptr<EndDeviceLoraPhy> edPhy =
            loraNetDevice->GetPhy()->GetObject<EndDeviceLoraPhy>();
        ...
        gatewayNetDevice->GetObject<LoraNetDevice> (); Ptr<GatewayLoraPhy>
        gatewayPhy = gatewayLoraNetDevice->GetPhy ()->GetObject<GatewayLoraPhy>
...
std::vector<int> LorawanMacHelper::SetSpreadingFactorsGivenDistribution(
    NodeContainer endDevices, NodeContainer gateways,
    ...
    NS_LOG_DEBUG("Distribution: " << distribution[0] << " " << distribution[1]
                                  ...
                                  << " " << distribution[5]);
    NS_LOG_DEBUG("Cumulative distribution: "
                 << cumdistr[0] << " " << cumdistr[1] << " " << cumdistr[2]
