 ...
 * \ingroup lorawan
 *
 * Helper class for configuring and installing the LorawanMac class on devices
 * and gateways. The user must set all parameters before calling Install on
 * nodes.
 */
class LorawanMacHelper {
  public:
     ...
     *
     * The Data Rate (DR) of each device is set to the maximum possible for its
     * transmissions to be correctly received by the gateway measuring the best
     * RSSI from the device, mimicking the DR maximization part of the default
     * online LoRaWAN Adaptive Data Rate (ADR) algorithm. Please note that a
     * single RSSI measurement between each device and gateway pair is taken for
     * the DR assignment, so the assignment may be suboptimal in scenarios with
     * a time-varying channel.
     *
     * This function uses the following convention (EU868) to compute the
     * transmission range:
     *
     * DR5 -> SF7  \n
     * DR4 -> SF8  \n
    ...
    /**
     * Randomly initialize the end devices' data rate parameter according to the
     * given distribution.
     *
     * This function expects a data rate (DR) distribution vector of length 6
     * filled with real numbers summing up to 1. The value at index \f$i\f$ is
     * considered to be the fraction of devices to be assigned DR \f$5-i\f$, for
     * example:
     *
     ...
     *
     *
     * Devices are then randomly assigned a DR following the provided
     * distribution.
     *
     * It returns a DR distribution vector with the following counters:
     *
     ...
     *
     * \param endDevices The end devices to configure.
     * \param gateways The gateways in the network (this is only a placeholder
     * parameter). \param distribution The distribution (probability mass
     * function) of DR assignment. \return A vector containing the final number
     * of devices per DR.
     *
     * \todo Remove unused parameter gateways.
     */
    static std::vector<int>
    SetSpreadingFactorsGivenDistribution(NodeContainer endDevices,
  ...
  private:
    ...
    /**
     * Apply configurations that are common both for the GatewayLorawanMac and
     * the ClassAEndDeviceLorawanMac classes.
     *
     ...
     */
    void
    ConfigureForSingleChannelRegion(Ptr<ClassAEndDeviceLorawanMac> edMac) const;
     ...
     */
    void
    ApplyCommonSingleChannelConfigurations(Ptr<LorawanMac> lorawanMac) const;
    ...
    Ptr<LoraDeviceAddressGenerator>
        m_addrGen;                //!< Pointer to the address generator to use
