...
TypeId ClassAEndDeviceLorawanMac::GetTypeId() {
...
ClassAEndDeviceLorawanMac::ClassAEndDeviceLorawanMac()
    : // LoraWAN default
      ...
      // LoraWAN default
      m_receiveDelay2(Seconds(2)), m_rx1DrOffset(0) {
...
void ClassAEndDeviceLorawanMac::SendToPhy(Ptr<Packet> packetToSend) {
    ...
    m_phy->GetObject<EndDeviceLoraPhy>()->SetFrequency(
        txChannel->GetFrequency());

    // Instruct the PHY on the right Spreading Factor to listen for during the
    // window create a SetReplyDataRate function?
    uint8_t replyDataRate = GetFirstReceiveWindowDataRate();
    NS_LOG_DEBUG("m_dataRate: "
                 << unsigned(m_dataRate)
...
// changement dynamique du Coding Rate, Cycle Redundancy Check (CRC) et Mode de
// trafic
...
void ClassAEndDeviceLorawanMac::FailedReception(Ptr<const Packet> packet) {
    NS_LOG_FUNCTION(this << packet);
...
void ClassAEndDeviceLorawanMac::TxFinished(Ptr<const Packet> packet) {
    NS_LOG_FUNCTION_NOARGS();
    ...
    m_secondReceiveWindow = Simulator::Schedule(
        m_receiveDelay2, &ClassAEndDeviceLorawanMac::OpenSecondReceiveWindow,
        this);
    // // Schedule the opening of the first receive window
    // Simulator::Schedule (m_receiveDelay1,
    //                      &ClassAEndDeviceLorawanMac::OpenFirstReceiveWindow,
...
void ClassAEndDeviceLorawanMac::OpenFirstReceiveWindow() {
    NS_LOG_FUNCTION_NOARGS();

    // Set Phy in Standby mode
    m_phy->GetObject<EndDeviceLoraPhy>()->SwitchToStandby();

    // Calculate the duration of a single symbol for the first receive window
    // data rate
...
void ClassAEndDeviceLorawanMac::CloseFirstReceiveWindow() {
    NS_LOG_FUNCTION_NOARGS();
    ...
    case EndDeviceLoraPhy::RX:
        // PHY is receiving: let it finish. The Receive method will switch it
        // back to SLEEP.
        break;
    case EndDeviceLoraPhy::SLEEP:
        // PHY has received, and the MAC's Receive already put the device to
        // sleep
        break;
...
void ClassAEndDeviceLorawanMac::OpenSecondReceiveWindow() {
    NS_LOG_FUNCTION_NOARGS();

    // Check for receiver status: if it's locked on a packet, don't open this
    // window at all.
    if (m_phy->GetObject<EndDeviceLoraPhy>()->GetState() ==
        EndDeviceLoraPhy::RX) {
        NS_LOG_INFO(
    ...
    NS_LOG_INFO("Using parameters: "
                << m_secondReceiveWindowFrequency << "Hz, DR"
    ...
    // Calculate the duration of a single symbol for the second receive window
    // data rate
...
void ClassAEndDeviceLorawanMac::CloseSecondReceiveWindow() {
    NS_LOG_FUNCTION_NOARGS();
    ...
    if (m_retxParams.waitingAck) {
        ...
        if (m_retxParams.retxLeft > 0) {
            ...
            this->Send(m_retxParams.packet);
                 ...
                 m_phy->GetObject<EndDeviceLoraPhy>()->GetState() !=
                     EndDeviceLoraPhy::RX) {
            uint8_t txs = m_maxNumbTx - (m_retxParams.retxLeft);
            m_requiredTxCallback(txs, false, m_retxParams.firstAttempt,
                                 m_retxParams.packet);
        ...
        NS_LOG_INFO("We have " << unsigned(m_retxParams.retxLeft)
                               << " transmissions left. We were not "
                                  "transmitting confirmed messages.");
...
Time ClassAEndDeviceLorawanMac::GetNextClassTransmissionDelay(
    Time waitingTime) {
    NS_LOG_FUNCTION_NOARGS();
    ...
    if (!m_retxParams.waitingAck) {
        if (!m_closeFirstWindow.IsExpired() ||
            ...
            // Compute the duration of a single symbol for the second receive
            // window data rate
            double tSym =
                pow(2, GetSfFromDataRate(GetSecondReceiveWindowDataRate())) /
                GetBandwidthFromDataRate(GetSecondReceiveWindowDataRate());
            ...
            waitingTime =
                std::max(waitingTime, endSecondRxWindow - Simulator::Now());
        }
    }
    ...
    // ACK_TIMEOUT (this timer starts when the second receive window was open)
        ...
        // Compute the duration until ACK_TIMEOUT (It may be a negative number,
        // but it doesn't matter.)
        Time retransmitWaitingTime = Time(m_secondReceiveWindow.GetTs()) -
...
uint8_t ClassAEndDeviceLorawanMac::GetFirstReceiveWindowDataRate() {
    return m_replyDataRateMatrix.at(m_dataRate).at(m_rx1DrOffset);
}

void ClassAEndDeviceLorawanMac::SetSecondReceiveWindowDataRate(
    uint8_t dataRate) {
    m_secondReceiveWindowDataRate = dataRate;
}

uint8_t ClassAEndDeviceLorawanMac::GetSecondReceiveWindowDataRate() const {
    return m_secondReceiveWindowDataRate;
}

void ClassAEndDeviceLorawanMac::SetSecondReceiveWindowFrequency(
    double frequencyMHz) {
    m_secondReceiveWindowFrequency = frequencyMHz;
}

double ClassAEndDeviceLorawanMac::GetSecondReceiveWindowFrequency() const {
...
void ClassAEndDeviceLorawanMac::OnRxClassParamSetupReq(
    Ptr<RxParamSetupReq> rxParamSetupReq) {
    ...
    m_macCommandList.emplace_back(
        CreateObject<RxParamSetupAns>(offsetOk, dataRateOk, true));
}
